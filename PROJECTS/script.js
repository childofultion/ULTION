// 360° carousel + video lightbox (mobile friendly, radius is responsive)
(function () {
  ...
  // Feel
  let RADIUS = 480; // will be computed below
  ...
  // --- responsive radius (prevents giant cards on phones) ---
  function computeRadius(){
    const w = carousel.clientWidth || window.innerWidth;
    const h = Math.min(window.innerHeight, window.innerWidth * 1.2);
    const isPhone = window.matchMedia('(max-width:700px)').matches;

    // slightly shallower wheel on phones
    RADIUS = (isPhone ? 0.52 : 0.42) * Math.min(w, h);
  }
  computeRadius();

  window.addEventListener('resize', () => {
    computeRadius();
    // re-render at same index with new depth
